...
FROM CovidDeaths
WHERE location IN ('Europe', 'Africa', 'North America', 'Asia', 'Australia', 'South America')
GROUP BY  location
ORDER BY 2 DESC
...
SELECT cd.continent, cd.location, cd.date, cd.population,
		cv.new_vaccinations, 
		SUM(cv.new_vaccinations) OVER (PARTITION BY cd.location ORDER BY cd.location, cd.date) as rolling_count_vaccinations
FROM CovidDeaths cd
